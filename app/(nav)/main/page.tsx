...
import {
  type Mission,
  listenToPersonalMissions,
  getActiveMission,
} from "@/lib/firebase/missions";
import {
  PermissionDialog,
  checkIfPermissionsAlreadyGranted,
} from "@/components/permission-dialog";
  ...
  return (
    <Card
      className={cn(
        ...
        isActive
          ? "ring-4 ring-[rgba(108,211,255,1)] ring-opacity-50 shadow-xl scale-105"
          : ""
      )}
      style={{
    ...
    >
      ...
      <CardContent className="flex mt-8 w-full flex-col items-center text-center">
        <h2 className="font-bold mb-2 text-[rgba(125,71,185,1)] text-4xl">
          {mission.title}
        </h2>
        <p className="max-w-[180px] mb-2 text-[rgba(174,121,235,1)] text-lg">
          {mission.description}
        </p>
        ...
        <Button
          ...
          style={{
            boxShadow: isFunctionallyActive
              ? "0 5px 0 #50B0FF"
              : "0 5px 0 #9CA3AF",
          }}
        >
...
export default function MainAppWireframe() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToPersonalMissions(
      user.uid,
      (fetchedMissions) => {
  ...
  return (
    ...
    >
          ...
          <SwipeableCardStack className="w-full">
            {missions.map((mission) => {
              ...
              const isFunctionallyActive =
                activeMission?.id === mission.id && !mission.status;
              return (
                <MissionCard
                  key={mission.id}
                  mission={mission}
      ...
      <footer className="w-full flex flex-col items-center p-3 z-10 bg-transparent pb-3 pt-20">
        <div className="text-center">
          <div className="text-5xl font-extrabold leading-none text-[rgba(125,71,185,1)]">
            {dayNumber}
          </div>

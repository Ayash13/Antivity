...
export default function UserProfilePage({
  ...
  useEffect(() => {
    ...
    const loadBadges = async () => {
      try {
        const [badgeList, missionCount] = await Promise.all([
          getBadges(),
          getCompletedMissionCount(uid),
        ]);
        setBadges(badgeList);
          ...
          <div className="flex items-center gap-4 px-6">
            ...
            <div className="flex-1 flex justify-around">
              <div className="text-center">
                <div className="text-lg font-bold text-[#7D47B9]">
                  Followers
                </div>
            ...
            <div className="px-6 pt-4 text-center">
              <button
                ...
                className={`py-3 px-6 rounded-full font-semibold text-sm transition-all duration-200 bg-[rgba(80,176,255,1)] w-full ${
                  isFollowingUser
                    ? "bg-[#6CD3FF] text-white"
                    : "bg-white text-[#50B0FF] hover:bg-gray-50"
                } ${
                  followLoading
              ...
              <div className="grid grid-cols-2 gap-2">
                {daySessions.map((s, idx) => (
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/profile/${uid}/photos/${encodeURIComponent(s.docId)}`
                      )
                    }

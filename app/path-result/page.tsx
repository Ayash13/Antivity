...
import { getLatestPathSession, getPathSessionByDocId, type PathSessionDoc } from "@/lib/firebase/path-logs"
...
export default function PathResultPage() {
  ...
  const getTotalDistance = (): number => {
    ...
    for (let i = 1; i < pathImages.length; i++) {
      ...
      if (prev.coord && curr.coord) {
        totalDistance += calculateDistance(prev.coord.lat, prev.coord.lng, curr.coord.lat, curr.coord.lng)
      }
    }
    ...
    try {
      ...
      captureBtn.onclick = () => {
        ...
        if (ctx) {
          ...
          ctx.scale(-1, 1)
          ctx.drawImage(video, sourceX, sourceY, sourceWidth, sourceHeight, -320, 0, 320, 320)
      ...
      try {
        ...
        if (pathImages.length === 0) {
          ...
          if (storedImages) {
            try {
              ...
              console.log("Loaded path images from sessionStorage:", parsedImages)
            } catch (e) {
        ...
        if (!session) {
          ...
          const [sessionData, profileData] = await Promise.all([
            sessionId ? getPathSessionByDocId(user.uid, sessionId) : getLatestPathSession(user.uid),
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (capturedSelfie) {
        ...
        const selfieStorageRef = ref(storage, `path_result/selfies/${selfieFileName}`)
      ...
      if (session?.docId) {
        const pathSessionRef = doc(db, `users/${user.uid}/pathSessions/${session.docId}`)
        await updateDoc(pathSessionRef, {
          selfieImageUrl: selfieUrl,
        })

        // Save journal entry without selfie URL
        const journalRef = collection(db, `users/${user.uid}/pathSessions/${session.docId}/journal`)
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `path-result-${Date.now()}.png`, { type: "image/png" })

      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Path Adventure",
          text: `${storyTitle || "My Adventure Story"} - I crushed ${getTotalDistance()} today!`,
          files: [file],
        })
      } else {
        ...
        if (navigator.clipboard) {
          await navigator.clipboard.writeText(
            `Check out my path adventure: ${storyTitle || "My Adventure Story"} - I crushed ${getTotalDistance()} today!`,
          )
        ...
        <div className="flex flex-col items-center w-full max-w-sm">
          <div
            ...
            style={{
              ...
              backgroundPosition: "center",
              borderRadius: "32px",
              transform: `scale(${Math.min(1, (window.innerWidth - 32) / 328)})`,
              transformOrigin: "center",
            }}
          >
            <div className="absolute inset-0 p-6 flex flex-col pb-5">
              <div className="absolute bottom-14 left-1/2 -translate-x-1/2 z-20">
                <img src="/images/star-path.webp" alt="Star path" className="h-12 w-auto" />
              </div>
                        ...
                        >
                          {capturedSelfie ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 text-gray-400">
                              ...
                              <span className="text-xs text-center">Tap to selfie</span>
                            </div>
                          )}
                        </div>
                      )
          ...
          <Button
            ...
            className={`w-full max-w-xs h-14 rounded-full text-white font-bold text-xl ${
              !capturedSelfie ? "bg-gray-400 cursor-not-allowed" : "bg-[rgba(108,211,255,1)] hover:bg-[#50B0FF]"
            }`}
            style={{
              boxShadow: !capturedSelfie ? "0 3px 0 #9CA3AF" : "0 3px 0 #50B0FF",
              ":hover": { boxShadow: !capturedSelfie ? "0 3px 0 #9CA3AF" : "0 3px 0 #50B0FF" },
            }}
          >
            {isSaving ? "Saving..." : !capturedSelfie ? "Take Selfie First" : "Save"}
          </Button>
        </div>
